...
```{r}
peak_table <- iso_files_w_peak_table %>% iso_get_peak_table() #create a dtaframe that updates whats happening in the peak table
file_info <- iso_files_w_peak_table %>% iso_get_file_info()
...
iso_files_w_file_info <- iso_files_w_peak_table %>% 
  ...
  iso_parse_file_info(number = c(seq_nr, analysis)) %>% #it can identify if you have some sort of sequence number
  # process specific sequence file information
  iso_mutate_file_info(
    # what is the type of each analysis? this purely depends of each run
    type = case_when(
      id1 == "empty"      ~ "empty",
      id1 == "blank"      ~ "blank",
    ...
    folder = basename(dirname(file_path)) #extracting the folder from the file
  ) %>% 
...
```{r}
iso_files_without_empty$`19613__acn1_79.dxf` %>% iso_plot_continuous_flow_data() #can see the run and you can verify if it is good or bad
```
...
iso_files %>% 
  ...
  iso_plot_continuous_flow_data(
    ...
    peak_label = sprintf("RT = %.3f", rt),#iso_format(id1, rt, d13C, signif = 3),
    peak_label_size = 3,
...
peak_table %>% 
  ...
  iso_mark_value_range(sd = c(1,2,3)) %>%  #you can use this with anything cause is ggplot
...
peak_table <- peak_table %>% 
  iso_mutate_peak_table(is_outlier = analysis %in% c(19691)) #you don't trow out the data only put is as an outlier 
```
...
calib_drift <- 
  ...
  iso_prepare_for_calibration() %>% #nesting
  ...
  iso_generate_calibration(
    ...
    use_in_calib = is_std_peak & type == "drift" & !is_outlier #you wnat everything that is standard peak and it not an outlier
  ) %>% 
...
calib_linearity <- 
  ...
  iso_generate_calibration(
    ...
    use_in_calib = is_std_peak & type == "linearity" & !is_outlier & area44 > 50 & area44 < 300
  ) %>% 
  # remove problematic calibrations if there are any
  iso_remove_problematic_calibrations()
```
...
#xamples of different cases

```{r}
# run a set of regressions to evaluate linearity
#split data by s signal range to apply different corrections
calib_linearity <- 
  peak_table_w_standards %>%
  ...
  # prepare for calibration
  iso_prepare_for_calibration(group_by = Signal_range) %>% 
  # run different calibrations
  iso_generate_calibration(
    calibration = "lin",
    # again evaluating different regression models of the deviation from the mean
...
```{r "linearity_residuals", fig.width = 8, fig.height=7}
# visualizing residuals
calib_linearity %>% iso_plot_residuals(x = area44, panel = Signal_range ~ lin_calib)
```

```{r "linearity_coefs", fig.width = 5, fig.height=8}
# show calibration coefficients
calib_linearity %>% iso_plot_calibration_parameters(shape = Signal_range)
```
...
calib_linearity_applied <- 
  ...
  filter(Signal_range == "low area" & lin_calib == "lm(d13_dev ~ 1)" | Signal_range == "high area" & lin_calib == "lm(d13C_dev ~ area44)") %>% 
