---
title: "Isoverse wokrshop"
subtitle: "SUBTITLE"
author: "Seb Kopf with Lina's comments"
...
di_files <- iso_read_dual_inlet(iso_get_reader_examples_folder(), nu_masses = 1:6, read_cache = FALSE)

# iso_read_dual_inlet("data") #so here you move all your raw files to the data folder of your repository and then just reads everything if you dowload the data and then move it
...
iso_save(di_files, filepath = "data_output/di_save") #need to create ahead a folder that is called data_output in your project
...
```{r}
#you can filter standards, blanks, etc... to subset
my_cf_files <- cf_files %>% 
  # filter files
  iso_filter_files(str_detect(file_id, "continuous")) %>% #you can use any aprameter here that is in the file and is information 
  # set peak table
  iso_set_peak_table_from_auto_vendor_data_table() %>% 
  # select relevant file info
  iso_select_file_info(
    file_datetime, 
    analysis = Analysis, id1 = `Identifier 1`, id2 = `Identifier 2`, #in R when you have a special character you want to put ' ' so R can read anything
    prep = Preparation, comment = Comment
  )

#iso_rename_file_info() #you can rename things in your file... you can rename certain things first and then you can finish analysis = Analysis, id1 = `Identifier 1`, everything() 
...
#you can save the last plot you made with ggsave()
```
...
iso_plot_continuous_flow_data(
  ...
  data= c("2", "3"),
  time_interval = c(500, 1000),#you cna setup what time fram you want to see
  color = data,
  #panel = NULL #when you dont want different panels and merge all the data in one especially when you have multiple files cause you get a panel per file
  panel = id1,
  peak_label = iso_format(rt, signif = 2), #you want to see only two significant figures in your plot of your retention time
  peak_label_size = 5,
  peak_label_filter = rt <700 # filter labels when you generate plots. 
  #peak_label_filter = between(rt, 500, 650) | between (rt, 800, 1000) # filter labels when you generate plots. alternating 
)
```
